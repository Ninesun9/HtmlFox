...
Imported files are copied into the app sandbox; the originals are never modified. Two files that share a name (for example `index.html` from different folders) currently share one sandbox slot, so opening the second one replaces the first in the Recent list. If you need to keep multiple copies side by side, rename one of them before opening.
